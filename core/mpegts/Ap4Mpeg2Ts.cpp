...
/*----------------------------------------------------------------------
 |   AP4_Mpeg2TsWriter::SampleStream::WriteMPEG2CCTO16
 +---------------------------------------------------------------------*/
void
AP4_Mpeg2TsWriter::Stream::WriteMPEG2PacketCCTO16(AP4_ByteStream& output) {
  while (GetCC()!= 0x0) {
...
void
AP4_Mpeg2TsWriter::WriteMPEG2PacketCCTO16(AP4_ByteStream& output) {
  if (m_Video) {
    m_Video->WriteMPEG2PacketCCTO16(output);
  }
  if (m_Audio) {
    m_Audio->WriteMPEG2PacketCCTO16(output);
  }
  while (m_PAT->GetCC() != 0x0) {
    WritePAT(output);
  }
  while (m_PMT->GetCC() != 0x0) {
    WritePMT(output);
  }
}
