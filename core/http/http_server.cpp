...
namespace Network
{
  ...
  namespace
  {
    ...
    private:
      ...
      virtual void SetResponseNotFound() {
        throw HttpRequestException(HTTP_NOTFOUND, "Could not find content for uri.");
      }
      virtual std::string RequestToString() {
        std::string host = GetHeaderAttr(Http::Request::Header::Host::Value);
        std::string ref  = GetHeaderAttr(Http::Request::Header::Referer::Value);
        const std::string path = GetPath();
        const IHttpRequest::Type reqType = GetRequestType();
        ...
        switch (reqType) {
          ...
          case IHttpRequest::Type::POST:
            reqTypeStr = "[POST]";
            break;
          case IHttpRequest::Type::OPTION:
            reqTypeStr = "[OPTION]";
            break;
          default:
            reqTypeStr = "[UNKNOWN REQ TYPE]";
            break;
        }
        SnyDataBuffer content; std::string post_content;
        const std::size_t content_size = GetContentSize();
        if (content_size <= 10240) {
          content.resize(static_cast<int>(content_size));
          GetContent(content.data(), content_size, false);
          post_content.append(content.data(), content.size());
        }
        return host + " - " + reqTypeStr + " - [ " + GetPath() +" ]" + " - [ " + post_content +" ]" + " - [" + std::to_string(ResponseCode) + " ]";
      }
    };
    ...
    void OnRawRequest(evhttp_request *request, void *prm)
    {
      try
      {
        ...
        ReqPrm->Func(Request, ReqPrm->user);
  ...
  HttpServer::HttpServer(std::string const &address, std::uint16_t port,
              std::uint16_t threadCount, OnRequestFunc const &onRequest, void* user,
