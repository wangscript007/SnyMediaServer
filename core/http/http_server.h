...
namespace Network
{
  ...
  public:
    ...
    typedef std::function<void (IHttpRequestPtr, void* user)> OnRequestFunc;
    ...
    HttpServer(std::string const &address, std::uint16_t port,
               std::uint16_t threadCount, OnRequestFunc const &onRequest, void* user = nullptr,
               MethodPool const &allowedMethods = {IHttpRequest::Type::GET , IHttpRequest::Type::POST},
